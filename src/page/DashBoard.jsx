...
function Dashboard() {
  ...
  const [files, setFiles] = useState(() => {
    const saved = localStorage.getItem('drive_files');
    return saved ? JSON.parse(saved) : [];
  });

  useEffect(() => {
    localStorage.setItem('drive_folders', JSON.stringify(folders));
  }, [folders]);
  ...
  return (
    ...
    >
      ...
      <div className="p-8 max-w-6xl mx-auto">
        ...
        <h2 className="text-2xl font-bold mb-6 text-indigo-950">Folders</h2>
        <div className="grid md:grid-cols-3 lg:grid-cols-4 gap-6 mb-12">
          {folders.length === 0 && (
            <div className="col-span-full py-12 px-6 text-center bg-white/50 border-2 border-dashed border-indigo-100 rounded-3xl backdrop-blur-sm">
              <p className="text-indigo-400 font-medium text-lg">No folders yet. Create one or drag & drop files here!</p>
            </div>
          )}
          {folders.map(folder => (
            <FolderCard 
        ...
        <div>
          ...
          {files.length === 0 ? (
            <div className="py-12 px-6 text-center bg-white/50 border-2 border-dashed border-indigo-100 rounded-3xl backdrop-blur-sm">
              <p className="text-indigo-400 font-medium text-lg">No standalone files yet.</p>
            </div>
          ) : (
